...
def preprocess_mrz(img: np.ndarray) -> np.ndarray:
    ...
    # Black-hat to pop dark characters on light background
...
def safe_int(digit: str) -> int:
    """Return int value of a check-digit; < becomes 0."""
    return 0 if digit == "<" else int(digit)


# -------------------------------------------------
# 5. TD-3 PARSER (Kenyan ID back uses TD-3 layout)
def parse_td3(line1: str, line2: str):
    ...
    checks = {
        ...
        "final_ok": mrz_checksum(
            passport + passport_cd + birth + birth_cd +
            expiry + expiry_cd + personal + personal_cd
        ) == safe_int(final_cd),
    }
...
async def mrz_endpoint(file: UploadFile = File(...)):
    ...
    raw = extract_mrz(img)
    if not raw:
        raise HTTPException(status_code=400, detail="No MRZ text detected")
